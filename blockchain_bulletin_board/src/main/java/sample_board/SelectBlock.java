...
public class SelectBlock extends HttpServlet {
  ...
  protected void doGet(HttpServletRequest request, HttpServletResponse response)
    ...
    BoardBlockDAO boardblockDAO = new BoardBlockDAO(); // 管理するためのインスタンスを生成
    List<BoardBlock> list = boardblockDAO.selectAllBoardBlock(); // 掲示板のデータをリストに挿入する
    request.setAttribute("list", list); // リクエストに対してリストをセットする
    RequestDispatcher rd = request.getRequestDispatcher("/WEB-INF/boardblock.jsp"); // board.jspを返す
